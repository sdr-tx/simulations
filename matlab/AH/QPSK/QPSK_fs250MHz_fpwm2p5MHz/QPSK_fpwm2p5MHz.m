...
load('QPSK_BPF_IIR_fp2p513MHz_fc2p7MHz_at60dB.mat');

%signal_filtered_FIR = filter(FIR_fc25MHz_fs23MHz,signal);
signal_filtered_IIR = filter(QPSK_BPF_IIR_fp2p513MHz_fc2p7MHz_at60dB,signal);


figure('name','Señales temporales')
h1=subplot(211),plot(t, signal); title('Señal temporal'); ylim([-0.5 1.5]); grid;
%h2=subplot(312),plot(t, signal_filtered_FIR); title('Señal filtrada con FIR'); ylim([-1 1]); grid;
h3=subplot(212),plot(t, signal_filtered_IIR); title('Señal filtrada con IIR'); ylim([-1 1]); grid;
linkaxes([h1,h3],'x');
...
h1=subplot(211), plot(f, 2*signal_spect(1:length(signal_spect)/2+1)); title('Signal Spectrum'),grid
%h2=subplot(312), plot(f, 2*signal_spect_FIR(1:length(signal_spect)/2+1)); title('Signal Spectrum FIR'),grid
h3=subplot(212), plot(f, 2*signal_spect_IIR(1:length(signal_spect)/2+1)); title('Signal Spectrum IIR'),grid
linkaxes([h1,h3],'x');
...
% h2 = subplot(212), 
plot(t,I_IIR,t,Q_IIR), grid, title('I Q IIR sin LPF')
...
load('QPSK_LPF_IIR_fp100k_fc500k_at60dB');
...
I_IIR_filtered = filter(QPSK_LPF_IIR_fp100k_fc500k_at60dB,I_IIR);
Q_IIR_filtered = filter(QPSK_LPF_IIR_fp100k_fc500k_at60dB,Q_IIR);

figure
% h1 = subplot(211); plot(t,I_FIR_filtered,t,Q_FIR_filtered), grid, title('I Q con FIR Filtrado')
% h2 = subplot(212); 
plot(t,I_IIR_filtered,t,Q_IIR_filtered), grid, title('I Q con IIR Filtrado')
...
% subplot(122);  
plot(t,message_IIR,'linewidth', 2); title('Mensaje en tiempo IIR');

% subplot(121); hist(message_FIR);    title('Histograma FIR');
% subplot(122);
hist(message_IIR);    title('Histograma IIR');
